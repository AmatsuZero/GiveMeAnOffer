...
class BinarySearchTreeTests: XCTestCase {
    ...
    func testRootNode() {
        let tree = BinarySearchTree(8)
        XCTAssertEqual(tree.count, 1)
        XCTAssertEqual(tree.minimum().val, 8)
        XCTAssertEqual(tree.maximum().val, 8)
    ...
    func testCreateFromArray() {
        ...
        XCTAssertEqual(tree.search(value: 9)!.val, 9)
        XCTAssertNil(tree.search(value: 99))
        
        XCTAssertEqual(tree.minimum().val, 3)
        XCTAssertEqual(tree.maximum().val, 16)
    ...
    func testInsert() {
        ...
        XCTAssertEqual(tree.minimum().val, 3)
        XCTAssertEqual(tree.maximum().val, 10)
    ...
    func testPredecessor() {
        ...
        XCTAssertEqual(node!.predecessor()!.val, 4)
        XCTAssertEqual(node!.predecessor()!.predecessor()!.val, 3)
        XCTAssertEqual(node!.predecessor()!.predecessor()!.predecessor()!.val, 2)
        XCTAssertEqual(node!.predecessor()!.predecessor()!.predecessor()!.predecessor()!.val, 1)
    ...
    func testSuccessor() {
        ...
        XCTAssertEqual(node!.successor()!.val, 2)
        XCTAssertEqual(node!.successor()!.successor()!.val, 3)
        XCTAssertEqual(node!.successor()!.successor()!.successor()!.val, 4)
        XCTAssertEqual(node!.successor()!.successor()!.successor()!.successor()!.val, 5)
        XCTAssertNil(node!.successor()!.successor()!.successor()!.successor()!.successor())
    }

    static var allTests = [
        ("由数组创建", testCreateFromArray),
        ("插入", testInsert),
        ("插入重复项", testInsertDuplicates),
        ("插入有序数组", testInsertSorted),
        ("前项", testPredecessor),
        ("移除叶子结点", testRemoveLeaf),
        ("移除一个左孩子", testRemoveOneChildLeft),
        ("移除一个右孩子", testRemoveOneChildRight),
        ("移除根节点", testRemoveRoot),
        ("RemoveTwoChildrenSimple", testRemoveTwoChildrenSimple),
        ("RemoveTwoChildrenComplex", testRemoveTwoChildrenComplex),
        ("根结点", testRootNode),
        ("后继", testSuccessor),
        ("遍历", testTraversing)
    ]
