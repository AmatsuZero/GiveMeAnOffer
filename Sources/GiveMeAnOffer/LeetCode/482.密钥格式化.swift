...
//
/*
 * @lc app=leetcode.cn id=482 lang=swift
 *
 * [482] 密钥格式化
 *
...
public extension String {
    func licenseKeyFormatting(_ k: Int) -> String {
        var S = uppercased().replacingOccurrences(of: "-", with: "")
        let len = S.count
        for i in stride(from: k, to: len, by: k) {
            let index = S.index(S.startIndex, offsetBy: len - i)
            S.insert("-", at: index)    
        }
        return S
    }
