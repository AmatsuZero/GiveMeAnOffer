...
extension String {
    ...
    public func romanToInt() -> Int {
        ...
        var result = 0, left = startIndex, right = index(after: left)
        while right < endIndex {
            let l = String.romanTable[self[left]]!, r = String.romanTable[self[right]]!
            if l < r {
                result += (r - l)
                left = index(after: right)
                if left == endIndex {
                    break
                }
                right = index(after: left)
            } else {
                result += l
        ...
        if left != endIndex {
            result += String.romanTable[self[left]]!
        }
        return result
    }
}
